...
log_message = function(level, msg)
    ...
    print(entry)
    logObj = computer.File(log_dir + "/daemon.log")
    if logObj == null then
        computer.touch(log_dir, "daemon.log")
        logObj = computer.File(log_dir + "/daemon.log")
    end if
    if logObj != null then
        existing = logObj.get_content
...
save_list = function(filePath, list)
    ...
    if file == null then
        ...
        fileName = parts[-1]
        folderPath = filePath[:filePath.len - fileName.len - 1]
        computer.touch(folderPath, fileName)
        file = computer.File(filePath)
    end if
    if file != null then
...
load_list = function(filePath)
    list = []
    file = computer.File(filePath)
    if file == null then return list
    content = file.get_content
    if content == null or content == "" then return list
    lines = content.split("\n")
    for line in lines
        line = line.trim
        if line != "" then
...
extract_ips = function(text)
    ...
    for word in words
        ...
        while word.len > 0 and not ((word[0] >= "0" and word[0] <= "9") or word[0] == ".")
            word = word[1:]
        end while
        while word.len > 0 and not ((word[word.len-1] >= "0" and word[word.len-1] <= "9") or word[word.len-1] == ".")
            word = word[:word.len-1]
        end while
        if word == "" then continue
        if is_valid_ip_str(word) then
            if ips.indexOf(word) == null then
                ips.push(word)
            end if
        end if
    end for
...
is_blacklisted = function(content)
    ...
    for keyword in blacklist_keywords
        if content.indexOf(keyword) != null then
            return true
        end if
    end for
    return false
...
process_emails = function()
    ...
    mail_account = mail_login(email_address, email_password)
    if not mail_account then
    ...
    if typeof(emails) != "list" then
        log_message("INFO", "fetch did not return a list: " + typeof(emails))
        return []
    end if
    log_message("INFO", "Fetched " + emails.len + " email(s)")
    new_targets = []
    for email in emails
        if email == null then continue
        if typeof(email) != "string" then continue
        if email.trim == "" then continue
        mailID = ""
        for line in email.split("\n")
            line = line.trim
            if line.indexOf("MailID:") == 0 then
                mailID = line[8:].trim
                break
            end if
        end for
        if mailID == "" then
            log_message("WARN", "Could not find MailID")
            continue
        end if
        log_message("DEBUG", "Reading MailID: " + mailID)
        fullContent = mail_account.read(mailID)
        if fullContent == null then
            log_message("WARN", "Could not read full email")
            continue
        end if
        if typeof(fullContent) != "string" then continue
        if fullContent.indexOf("Mission Contract") == null then
            log_message("DEBUG", "No 'Mission Contract' in email, ignoring")
            continue
        end if
        if is_blacklisted(fullContent) then
            log_message("INFO", "Blacklisted email skipped")
            continue
        end if
        ...
        for ip in ips
            ...
            if completed_scans.indexOf(ip) == null and scan_queue.indexOf(ip) == null then
                scan_queue.push(ip)
                new_targets.push(ip)
                log_message("INFO", "Queued target: " + ip)
            else
                log_message("DEBUG", "Already queued or completed: " + ip)
            end if
        end for
    end for
...
run_mapkit = function(target_ip)
    if computer.File("/bin/mapkit") == null then
        log_message("ERROR", "mapkit not found at /bin/mapkit")
        return false
    end if
    wait(2)
    log_message("INFO", "Launching mapkit for: " + target_ip)
    shell.launch("/usr/bin/Terminal.exe", "/bin/mapkit " + target_ip)
    wait(180)
    if computer.File(targets_dir + "/" + target_ip + ".txt") != null then
        log_message("INFO", "Mapkit completed for " + target_ip)
        return true
    end if
...
execute_scan_queue = function()
    ...
    if run_mapkit(target_ip) then
        completed_scans.push(target_ip)
        save_list(completed_file, completed_scans)
        log_message("INFO", "Scan done: " + target_ip)
        try_harvest(target_ip, targets_dir)
    else
        log_message("WARN", "Mapkit failed for: " + target_ip)
        scan_queue.push(target_ip)
    end if
    save_list(queue_file, scan_queue)
end function
// ==================== ARGUMENT PARSING ====================
...
if mypid != null then
    ...
    if saved then log_message("INFO", "Saved current PID: " + mypid)
else
...
while true
    ...
    if now - last_check >= 300 then
        ...
        new_targets = process_emails()
        if new_targets.len > 0 then
            log_message("INFO", "New targets: " + new_targets.join(", "))
        end if
    end if
    if scan_queue.len > 0 then
        execute_scan_queue()
    ...
    if auto_mode and scan_queue.len == 0 then
        log_message("INFO", "Auto mode: queue empty. Waiting 10 seconds before relaunch...")
        wait(10)
