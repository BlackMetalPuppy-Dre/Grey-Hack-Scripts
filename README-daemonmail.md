Main Control Flow:
command-line options such as --once, --auto, --interval, and --old-pid. --once i never use (more debug then function will be removed) i always run with --auto (will check terminal PID saves it for after mail scan) 
--interval is going to be removed, and --old-pid is for the auto flag so it can kill the old terminal (because gray hack mail is weird the terminal does not see new mails) thats why the PID kill
