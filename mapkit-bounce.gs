...
wait(0.2)
print("Starting...")
wait(0.2) //you can delete this if you dont want this startup sequence! 
...
if targetFolderObj != null and targetFolderObj.is_folder then
    for libDir in targetFolderObj.get_folders
        ...
        if not libMatchesTarget then continue
...
if libFolders.len == 0 then
    print("<b>Warning: No matching libraries found. Showing all.</b>")
    for f in folderList
        if f.is_folder then
            libFolders.push(f)
            libFolderNames.push(f.name)
...
while true
    ...
    while true
        ...
        if zone == "" or exploitName == "" or port == "" then
            print("<b>Error: Could not parse vuln file.</b>")
            continue
        end if
        ...
        portInput = user_input("Port (Enter to use " + port + "): ").trim
        ...
        if not net_session then
            print("<b>Error: Could not connect.</b>")
            continue
        end if
        ...
        if result == null then
            print("<b>Exploit failed.</b>")
            wait(0.5)
            continue
        else if typeof(result) == "shell" then
            print("<b>Shell obtained on " + ipAddress + "!</b>")
            wait(0.5)
            tgt_shell = result
            tgt_comp = tgt_shell.host_computer
            tgt_shell.start_terminal
            ...
            print("Result: File access obtained!")
            print("File: " + result.path)
            wait(0.5)
            continue
        else if typeof(result) == "computer" then
            print("Result: Computer access obtained!")
            wait(0.5)
            continue
        else
            print("<b>Exploit failed (" + typeof(result) + ").</b>")
            wait(0.5)
            continue
        end if
    end while
